...
left join
 ...
 create or replace view health_card_index_state as
select 'basic_details' as data_source,row_to_json(basic_details)::jsonb as values  from 
...
(select 'udise'as data,row_to_json(udise)::jsonb from (select sum(total_schools) as total_schools
  ,cast(avg(administration)as int)as administration,cast(avg(community_participation)as int)as community_participation,
    cast(avg(enrollment)as int)as enrollment,cast(avg(grant_expenditure)as int)as grant_expenditure,
    cast(avg(nsqf)as int)as nsqf,cast(avg(policy_implementation)as int)as policy_implementation,
    cast(avg(safety)as int)as safety,cast(avg(school_infrastructure)as int)as school_infrastructure,
    cast(avg(school_inspection)as int)as school_inspection,cast(avg(school_performance)as int)as school_performance,
    cast(avg(teacher_profile)as int)as teacher_profile,cast(avg(arts_lab_index)as int)as arts_lab_index,
    cast(avg(ict_lab_index)as int)as ict_lab_index,cast(avg(medical_index)as int)as medical_index,cast(avg(science_lab_index)as int)as science_lab_index,
 ...
 sum(case when Infrastructure_score >75 then 1 else 0 end)as value_above_75
  ...
  union
 (select 'school_infrastructure'as data,row_to_json(infra)::jsonb from (select sum(total_schools_data_received) as total_schools_data_received, 
   cast(avg(infra_score)as int)as infra_score,
cast(avg(average_value)as int)as average_value,
 cast(avg(average_percent)as int)as average_percent, cast(avg(toilet_value)as int)as toilet_value, cast(avg(toilet_percent)as int)as toilet_percent,
  cast(avg(handwash_value)as int)as handwash_value,
cast(avg(handwash_percent)as int)as handwash_percent, cast(avg(solar_panel_value)as int)as solar_panel_value,
cast(avg(solar_panel_percent)as int)as solar_panel_percent, cast(avg(library_value)as int)as library_value,
cast(avg(library_percent)as int)as library_percent, cast(avg(drinking_water_value)as int)as drinking_water_value,
cast(avg(drinking_water_percent)as int)as drinking_water_percent, cast(avg(tap_water_value)as int)as tap_water_value,
cast(avg(tap_water_percent)as int)as tap_water_percent, cast(avg(electricity_value)as int)as electricity_value,
cast(avg(electricity_percent)as int)as electricity_percent, cast(avg(hand_pumps_value)as int)as hand_pumps_value,
cast(avg(hand_pumps_percent)as int)as hand_pumps_percent, cast(avg(boys_toilet_value)as int)as boys_toilet_value,
cast(avg(boys_toilet_percent)as int)as boys_toilet_percent, cast(avg(girls_toilet_value)as int)as girls_toilet_value,
 cast(avg(girls_toilet_percent)as int)as girls_toilet_percent,
 sum(case when infra_score <=33 then 1 else 0 end)as value_below_33,
 sum(case when infra_score > 33 and infra_score<= 60 then 1 else 0 end)as value_between_33_60,
 sum(case when infra_score > 60 and infra_score<= 75 then 1 else 0 end)as value_between_60_75,
 sum(case when infra_score >75 then 1 else 0 end)as value_above_75
